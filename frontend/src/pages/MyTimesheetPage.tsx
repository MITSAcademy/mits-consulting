...
export function MyTimesheetPage() {
  ...
  const { data: jobCodes = [] } = useQuery<any[]>({
    queryKey: ['job-codes'],
  ...
  const { data: entries = [], isLoading } = useQuery<any[]>({
    ...
    queryFn: () => api.get('/timesheet/entries', { params: { date, userId: user?.id } }).then((r) => r.data),
    enabled: !!user,
  });

  const { from, to } = weekRange(date);
  const { data: weekEntries = [] } = useQuery<any[]>({
    queryKey: ['timesheet-entries-week', from, to, user?.id],
    queryFn: () => api.get('/timesheet/entries', { params: { from, to, userId: user?.id } }).then((r) => r.data),
    enabled: !!user,
  });

  function inv() {
    qc.invalidateQueries({ queryKey: ['timesheet-entries'] });
    qc.invalidateQueries({ queryKey: ['timesheet-entries-week'] });
  }
    ...
    <>
      ...
      <Page>
        ...
        <div className="card mb-4">
          ...
          {showAdd && (
            <div
              className="mt-3 p-3 rounded-lg flex flex-wrap gap-2 items-end"
            ...
            >
              <div style={{ minWidth: 180 }}>
                ...
                <select
                  className="input text-sm"
                  value={addForm.jobCodeId}
                  onChange={(e) => setAddForm((f) => ({ ...f, jobCodeId: e.target.value }))}
                >
                  <option value="">Select…</option>
                  {activeJobCodes.map((jc: any) => (
              ...
              <Button
                size="sm"
                onClick={() => createMut.mutate({ date, jobCodeId: addForm.jobCodeId, hours: Number(addForm.hours), description: addForm.description })}
                disabled={!addForm.jobCodeId || !addForm.hours || !addForm.description || createMut.isPending}
              >
                Save
              </Button>
                  ...
                  {editId === e.id ? (
                    <div className="flex flex-wrap gap-2 items-end">
                      <div style={{ minWidth: 180 }}>
                        <select
                          className="input text-sm"
                          value={editForm.jobCodeId}
                          onChange={(ev) => setEditForm((f) => ({ ...f, jobCodeId: ev.target.value }))}
                        >
                          {activeJobCodes.map((jc: any) => (
                            <option key={jc.id} value={jc.id}>{jc.code} – {jc.name}</option>
                          ))}
                      ...
                      <Button
                        size="sm"
                        onClick={() => editMut.mutate({ id: e.id, data: { jobCodeId: editForm.jobCodeId, hours: Number(editForm.hours), description: editForm.description } })}
                    ...
                    <div className="flex items-start gap-3 flex-wrap">
                      <span
                        className="text-[11px] font-mono font-bold px-2 py-0.5 rounded flex-shrink-0"
                        style={{ background: 'var(--brand-accent)22', color: 'var(--brand-accent)', border: '1px solid var(--brand-accent)44' }}
                      >
                        {e.jobCode.code}
                      </span>
                      <span className="text-sm font-semibold flex-shrink-0" style={{ color: 'var(--brand-text)' }}>{e.hours}h</span>
                      <span className="text-sm flex-1" style={{ color: 'var(--brand-text)', minWidth: 120 }}>{e.description}</span>
                      <StatusBadge status={e.status} />
                      {e.status === 'rejected' && e.rejectionNote && (
                        <div
                          className="w-full text-xs mt-1 px-2 py-1 rounded"
                          style={{ color: 'var(--status-red)', background: 'rgba(239,68,68,0.08)' }}
                        >
                      ...
                      <div className="flex items-center gap-1 ml-auto flex-shrink-0">
                        {(e.status === 'draft' || e.status === 'rejected') && (
                          <>
                            <Button
                              ...
                              onClick={() => {
                                setEditId(e.id);
                                setEditForm({ jobCodeId: e.jobCodeId, hours: String(e.hours), description: e.description });
                              }}
                            >
                              <Edit2 size={13} />
                            </Button>
                        ...
                        {(e.status === 'draft' || e.status === 'rejected') && (
                          <Button
                            size="sm"
                            title={e.status === 'rejected' ? 'Re-submit' : 'Submit for approval'}
                            onClick={() => submitMut.mutate(e.id)}
                            disabled={submitMut.isPending}
                          >
                            <Send size={13} className="mr-1" />
                            {e.status === 'rejected' ? 'Re-submit' : 'Submit'}
                          </Button>
                        )}
                        {e.status === 'submitted' && (
                        ...
                        {e.status === 'approved' && (
                          <span className="text-xs" style={{ color: 'var(--status-green)' }}>
                            ✓ Approved{e.approvedBy ? ` by ${e.approvedBy.name}` : ''}
                          </span>
                        )}
                      </div>
                    </div>
        ...
        <div className="card">
          ...
          <div className="flex gap-2 flex-wrap mt-2">
            {weekDaysList.map((d, i) => (
              <button
                ...
                style={{
                  ...
                  background: d === date ? 'rgba(229,178,76,0.1)' : 'var(--bg-card)',
            ...
            <div
              className="flex flex-col items-center gap-1 px-3 py-2 rounded-lg ml-auto"
              style={{ border: '1px solid rgba(229,178,76,0.3)', background: 'rgba(229,178,76,0.08)', minWidth: 52 }}
            >
