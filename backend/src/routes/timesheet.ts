...
// GET /timesheet/job-codes — returns ALL codes (active + inactive); frontend filters for dropdown
timesheetRouter.get('/job-codes', async (_req, res) => {
...
timesheetRouter.post('/entries', async (req: AuthedRequest, res) => {
  ...
  if (!hours || Number(hours) <= 0 || Number(hours) > 24) return res.status(400).json({ error: 'hours must be between 0 and 24' });
...
timesheetRouter.patch('/entries/:id', async (req: AuthedRequest, res) => {
  ...
  if (hours !== undefined) {
    if (Number(hours) <= 0 || Number(hours) > 24) return res.status(400).json({ error: 'hours must be between 0 and 24' });
...
// POST /timesheet/entries/:id/submit
timesheetRouter.post('/entries/:id/submit', async (req: AuthedRequest, res) => {
  const user = req.user!;
  const entry = await prisma.timesheetEntry.findUnique({ where: { id: req.params.id } });
  if (!entry) return res.status(404).json({ error: 'Not found' });
  if (entry.userId !== user.id) return res.status(403).json({ error: 'Not your entry' });
  if (entry.status !== 'draft' && entry.status !== 'rejected') return res.status(400).json({ error: 'Entry must be in draft or rejected status' });
  const updated = await prisma.timesheetEntry.update({
    where: { id: req.params.id },
    data: { status: 'submitted', rejectionNote: null },
    include: { jobCode: { select: { code: true, name: true } }, user: { select: { name: true } }, approvedBy: { select: { name: true } } },
  });
...
timesheetRouter.post('/entries/:id/approve', async (req: AuthedRequest, res) => {
  ...
  const entry = await prisma.timesheetEntry.findUnique({ where: { id: req.params.id } });
  if (!entry) return res.status(404).json({ error: 'Not found' });
  if (entry.status !== 'submitted') return res.status(400).json({ error: 'Entry must be submitted' });
  const updated = await prisma.timesheetEntry.update({
    where: { id: req.params.id },
...
timesheetRouter.get('/summary', async (req: AuthedRequest, res) => {
  ...
  const map: Record<string, { userId: string; userName: string; totalHours: number; submittedCount: number; approvedCount: number; draftCount: number; rejectedCount: number }> = {};
